...
const ProductsAdmin = () => {
  ...
  const updateProducts = () => {
    ...
    fetch(config.apiUrl + "products/" + config.operatorId, requestOptions)
      .then((response) => {
        return response.json();
      })
      .then((result) => {
        // this.setState({ productList: result.data.map((product) => { return product; }) });
        let productList = result.data.map((product) => {
          return product;
        });
        let rowData;
            ...
            if (p.active) {
              button = (
                <button
                  ...
                  onClick={() => disable(p)}
                >
                  <i className="fas fa-eye-slash"></i> Deshabilitar
                </button>
              );
            } else {
              button = (
                <button className="btn btn-primary" onClick={() => enable(p)}>
                  <i className="fas fa-eye"></i> Habilitar
                </button>
              );
            }
            ...
            return (
              <tr>
                ...
                <td className="d-flex justify-content-between">
                  {button}
                  <button className="btn btn-warning" onClick={() => edit(p)}>
  ...
  const disable = (product) => {
    if (
      window.confirm("¿Está seguro/a de querer deshabilitar:\n" + product.name)
    ) {
      ...
      fetch(config.apiUrl + "products/" + product.id, requestOptionsPatch)
        .then((response) => {
          return response.json();
        })
        .then((result) => {
  ...
  const enable = (product) => {
    if (
      window.confirm(
        "¿Está seguro/a de querer volver a habilitar:\n" + product.name
      )
    ) {
      const requestOptionsPatch = {
        method: "PUT",
